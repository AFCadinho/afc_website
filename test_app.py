...
with app.app_context():
    ...
    db.create_all()
    adinho = Users(
        name="adinho", password="$2b$12$j1l8HthnWaFygkd92x4L1uPl0dT4R3vR/YSB6Pc2El/FjZ8EN.vOu", is_admin=True)
    db.session.add(adinho)
    db.session.commit()

    pokemmo = Games(name="PokeMMO")
    db.session.add(pokemmo)
    db.session.commit()

    pokemon_blaze_online = Games(name="Pokemon Blaze Online")
    db.session.add(pokemon_blaze_online)
    db.session.commit()

    pokemon_brick_bronze = Games(name="Pokemon Brick Bronze")
    db.session.add(pokemon_brick_bronze)
    db.session.commit()
